...
pub fn get_cam_params
(
    orbit_controls: Arc<Mutex<OrbitControls>>,
)
-> CameraParams
{
    let mut r = orbit_controls.lock().unwrap();
    CameraParams {
        position: r.position.to_vec(),
        view_matrix: r.view_matrix(),
        projection_matrix: r.camera.projection_matrix,
    }
}